...
public class BZip2Engine : IArchiveFormat
{
    private readonly ILogger _log = Log.ForContext<BZip2Engine>();
    ...
    public async Task<Archive> OpenAsync(string path, Stream stream, AccessMode mode, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (Password != null)
            {
                _log.Warning("Password set on BZip2 archive which does not support native encryption");
            ...
            _log.Debug("Decompressing BZip2 data from {Path}", path);
            using var decompressor = SharpCompress.Compressors.BZip2.BZip2Stream.Create(
                readStream, CompressionMode.Decompress, false, true, false);
        ...
        {
            _log.Error(ex, "Failed to open BZip2 archive {Path}", path);
            throw;
        }
    }
                                ...
                                IProgress<ProgressReport>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                _log.Debug("Encryption enabled for BZip2 save, algorithm {Algorithm}", options.Encryption.Algorithm);
            ...
            foreach (var node in archive.Vfs.Root.Children)
            {
                ...
                using var compressor = SharpCompress.Compressors.BZip2.BZip2Stream.Create(
                    writeStream, CompressionMode.Compress, false, true, false);
            ...
            _log.Information("Saved BZip2 archive to stream");
        }
        catch (Exception ex)
        {
            _log.Error(ex, "Failed to save BZip2 archive");
            throw;
        }
    }
}
