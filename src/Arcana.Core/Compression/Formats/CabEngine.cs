...
public class CabEngine : IArchiveFormat
{
    private readonly ILogger _log = Log.ForContext<CabEngine>();
    public string Name => "CAB";
    public string Extension => ".cab";
    ...
    public async Task<Archive> OpenAsync(string path, Stream stream, AccessMode mode, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var descriptorId = descriptor?.Id ?? "Cab";
        ...
        {
            _log.Error(ex, "Failed to open CAB archive {Path}", path);
            throw;
        }
    }
    ...
    public void Save(Archive archive, Stream stream, CompressionOptions options,
        ...
        => throw new NotSupportedException("CAB writing is not supported");
