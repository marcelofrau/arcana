...
public class LzmaEngine : IArchiveFormat
{
    private readonly ILogger _log = Log.ForContext<LzmaEngine>();
    public string Name => "LZMA";
    public string Extension => ".lzma";
    ...
    public async Task<Archive> OpenAsync(string path, Stream stream, AccessMode mode, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (Password != null)
            {
                _log.Warning("Password set on LZMA archive which does not support native encryption");
            ...
            _log.Debug("Decompressing LZMA data from {Path}", path);
        ...
        {
            _log.Error(ex, "Failed to open LZMA archive {Path}", path);
            throw;
        }
    }
                                ...
                                IProgress<ProgressReport>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                _log.Debug("Encryption enabled for LZMA save, algorithm {Algorithm}", options.Encryption.Algorithm);
            ...
            _log.Information("Saved LZMA archive to stream");
        }
        catch (Exception ex)
        {
            _log.Error(ex, "Failed to save LZMA archive");
            throw;
        }
    }
}
